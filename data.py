...
def preprocess_adj_dense(adj):
    """Preprocessing of adjacency matrix for simple GCN model and conversion to tuple representation."""
    adj_normalized = normalize_adj_dense(adj + np.eye(adj.shape[0]))
    return adj_normalized

def normalize_torch_adj_dense(adj):
    """Symmetrically normalize adjacency matrix."""
    #adj = sp.coo_matrix(adj)
    rowsum = np.array(adj.sum(1)) # D
    d_inv_sqrt = np.power(rowsum, -0.5).flatten() # D^-0.5
    d_inv_sqrt[np.isinf(d_inv_sqrt)] = 0.
    d_mat_inv_sqrt = torch.diag(torch.tensor(d_inv_sqrt)) # D^-0.5
    #print(d_mat_inv_sqrt)
    return   (d_mat_inv_sqrt @ adj) @ d_mat_inv_sqrt #torch.tensor(adj.dot(d_mat_inv_sqrt).transpose().dot(d_mat_inv_sqrt)) # D^-0.5AD^0.5

def preprocess_torch_adj_dense(adj):
    """Preprocessing of adjacency matrix for simple GCN model and conversion to tuple representation."""
    adj_normalized = normalize_torch_adj_dense(adj + np.eye(adj.shape[0]))
    return adj_normalized.float()
